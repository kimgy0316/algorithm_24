...
class Contact:
    def __init__(self, phone_number, password, reservations=None):
        ...
        self.password = self.simple_hash(password)  # 조건3: 해싱 알고리즘 사용
        self.reservations = reservations if reservations is not None else []

    def simple_hash(self, password):
        return ''.join(chr(ord(char) + 3) for char in password)  # 간단한 문자열 변환 해싱

    def print_info(self):
        return f"전화번호: {self.phone_number}, 비밀번호 해시: {self.password}\n"
...
class BookingSystem:
    ...
    def sort_movies(self):
        # 조건2: 삽입 알고리즘을 사용하여 정렬
        for i in range(1, len(self.movies)):
            key = self.movies[i]
            j = i - 1
            while j >= 0 and key.title < self.movies[j].title:
    ...
    def linear_search_movie_by_title(self, title):
        # 조건1: 선형 검색 알고리즘으로 영화 정보 검색
        for idx, movie in enumerate(self.movies):
            if movie.title == title:
                return idx
        return -1

    def edit_movie(self, title, new_title, new_times, new_theater, new_age_limit):
        idx = self.linear_search_movie_by_title(title)  # 선형 검색 사용
        if idx != -1:
...
class Application(tk.Tk):
    ...
    def edit_movie_details(self, movie, details_window):
        ...
        if new_title is not None and new_times is not None and new_theater is not None and new_age_limit is not None:
            if self.booking_system.edit_movie(movie.title, new_title, new_times, new_theater, new_age_limit):
                self.update_movie_list()
    ...
    def delete_movie_details(self, movie, details_window):
        ...
        if confirm:
            if self.booking_system.delete_movie(movie.title):
                self.update_movie_list()
    ...
    def user_login(self):
        ...
        password = self.simple_hash(self.password_entry.get())  # 해싱 알고리즘 사용
    ...
    def view_reservations(self):
        ...
        for i in range(len(reservations)):
            ...
            for j in range(i+1, len(reservations)):
                if reservations[min_idx]["날짜"] > reservations[j]["날짜"]:
                    min_idx = j
            reservations[i], reservations[min_idx] = reservations[min_idx], reservations[i]
    ...
    def update_contact(self, phone_number, new_password):
        for contact in self.contact_list:
            if contact.phone_number == phone_number:
                contact.password = self.simple_hash(new_password)
