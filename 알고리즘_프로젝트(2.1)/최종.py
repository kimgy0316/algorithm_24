...
class BookingSystem:
    ...
    def load_movies(self):import os
import tkinter as tk
from tkinter import ttk, messagebox, simpledialog
import datetime
import re
...
class Movie:
    def __init__(self, title, times, theater, age_limit):
        ...
        self.theater = theater
        self.age_limit = age_limit
        self.seats = {time: ['A1', 'A2', 'A3', 'A4', 'A5', 'A6',
                             'B1', 'B2', 'B3', 'B4', 'B5', 'B6',
                             'C1', 'C2', 'C3', 'C4', 'C5', 'C6',
                             'D1', 'D2', 'D3', 'D4', 'D5', 'D6',
                             'E1', 'E2', 'E3', 'E4', 'E5', 'E6',
                             'F1', 'F2', 'F3', 'F4', 'F5', 'F6'] for time in times}
...
class BookingSystem:
    ...
    def display_movies(self):
        return "\n".join(f"{idx + 1}. {movie.title} ({movie.age_limit})" for idx, movie in enumerate(self.movies))
...
class ReservationSystem:
    ...
    def login_frame(self):
        ...
        login_window.title("사용자 로그인")

        ttk.Label(login_window, text="영화 예매 시스템에 오신 것을 환영합니다").grid(row=0, column=0, columnspan=2, pady=10)

        ttk.Label(login_window, text="전화번호를 입력하세요 (예: 010-1234-5678):").grid(row=1, column=0, sticky='e')
        self.phone_entry = ttk.Entry(login_window)
        self.phone_entry.grid(row=1, column=1)

        ttk.Label(login_window, text="비밀번호를 입력하세요:").grid(row=2, column=0, sticky='e')
        self.password_entry = ttk.Entry(login_window, show="*")
        self.password_entry.grid(row=2, column=1)

        login_button = ttk.Button(login_window, text="로그인", command=lambda: self.login(login_window))
        login_button.grid(row=3, column=0, columnspan=2, pady=10)
    ...
    def register_frame(self):
        ...
        ttk.Label(register_window, text="회원가입").grid(row=0, column=0, columnspan=2, pady=10)

        ttk.Label(register_window, text="전화번호를 입력하세요 (예: 010-1234-5678):").grid(row=1, column=0, sticky='e')
        self.register_phone_entry = ttk.Entry(register_window)
        self.register_phone_entry.grid(row=1, column=1)

        ttk.Label(register_window, text="비밀번호를 입력하세요:").grid(row=2, column=0, sticky='e')
        self.register_password_entry = ttk.Entry(register_window, show="*")
        self.register_password_entry.grid(row=2, column=1)

        ttk.Label(register_window, text="비밀번호를 다시 입력하세요:").grid(row=3, column=0, sticky='e')
        self.register_confirm_password_entry = ttk.Entry(register_window, show="*")
        self.register_confirm_password_entry.grid(row=3, column=1)

        register_button = ttk.Button(register_window, text="가입", command=lambda: self.register(register_window))
        register_button.grid(row=4, column=0, columnspan=2, pady=10)    
        ...
        if re.match(r"010-\d{4}-\d{4}$", phone_number):
            if phone_number in self.reservations:
                if self.reservations[phone_number]["password"] == password:
                    ...
                    messagebox.showerror("오류", "비밀번호가 일치하지 않습니다.", parent=window)
            else:
                messagebox.showerror("오류", "가입되지 않은 전화번호입니다.", parent=window)
        else:
            messagebox.showerror("오류", "전화번호 형식이 올바르지 않습니다. 다시 입력하세요.", parent=window)    
    ...
    def select_date(self):
        ...
        confirm_button = ttk.Button(self.main_frame, text="확인", command=self.select_time)
...
# Application 클래스: Tkinter를 사용한 애플리케이션의 GUI
class Application(tk.Tk):
    def __init__(self, booking_system, reservation_system, contact_list):
        super().__init__()
        self.booking_system = booking_system
        ...
        self.user_login_button = tk.Button(self, text="사용자 로그인", command=self.user_login)
        self.user_login_button.pack(pady=10)

        self.register_button = tk.Button(self, text="회원가입", command=self.register_user)
    ...
    def edit_movie_details(self, movie, details_window):
        ...
        new_theater = simpledialog.askstring("영화 수정", "새 상영관을 입력하세요:", initialvalue=movie.theater)
        new_age_limit = simpledialog.askstring("영화 수정", "새 연령 제한을 입력하세요:", initialvalue=movie.age_limit)
        if new_title and new_times and new_theater and new_age_limit:
            if self.booking_system.edit_movie(self.booking_system.movies.index(movie), new_title, new_times, new_theater, new_age_limit):
                self.update_movie_list()
                messagebox.showinfo("성공", "영화가 성공적으로 수정되었습니다")
                details_window.destroy()
    ...
    def user_login(self):
        self.reservation_system.login_frame()

    def register_user(self):
        self.reservation_system.register_frame()
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    root.title("영화 예매 시스템")
    reservation_system = ReservationSystem(root, file_path, r"C:\Users\LG\Desktop\reservations.txt")
    app = Application(booking_system, reservation_system, contact_list)
    app.mainloop()
        ...
        with open(self.movies_file, 'r', encoding='utf-8') as file:
            for line in file:
                ...
                title = movie_data[0]
                times = movie_data[1:]
                self.movies.append(Movie(title, times))
